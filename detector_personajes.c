...
void preguntar_sabor(char* sabor){
	printf("Cual es su sabor favorito?\n");
	mostrar_convencion_sabores();
	scanf(" %c", sabor);

	while(!(es_sabor_valido(*sabor))){
		printf("Ingrese un sabor valido\n");
		mostrar_convencion_sabores();
		scanf(" %c", sabor);
	}
}
...
void preguntar_talla(int* talla){
	printf("Cual es su talla de zapatos:\n");
	mostrar_tallas();
	scanf("%i", talla);

	while(es_talla_invalida(*talla)){
			printf("Ingrese una talla valida\n");
			mostrar_tallas();
			scanf("%i", talla);
		}
}
...
void preguntar_color(char* color){
	...
	while(!(es_color_valido(*color))){
		printf("Ingreso un color invalido, por favor ingrese un color valido:\n");
		mostrar_convecion_colores();
		scanf(" %c", color);
	}
}
...
void preguntar_altura(int* altura){
	printf("Cual su altura en cm, debe ser entre %icm y %icm:\n", ALTURA_MINIMA, ALTURA_MAXIMA);
	scanf("%i", altura);

	while(es_altura_invalida(*altura)){
		printf("Altura invalida, ingrese una altura entre %icm y %icm:\n", ALTURA_MINIMA, ALTURA_MAXIMA);
		scanf("%i", altura);
	}
...
*/
void detectar_coraje_o_johnny(int* altura, int* diferencia_altura_1, int* diferencia_altura_2, int resultado){
...
*/
void detectar_pollito_o_hueso(int* altura, int* diferencia_altura_1, int* diferencia_altura_2, int resultado){
...
/POST: Muestra por pantalla quien es el personaje detectado, que deberan ser BLUE o BELLOTA
 dependiendo de la comparacion de diferencia_altura_1 y diferencia_altura_2.
*/
void detectar_bellota_o_blue(int* altura, int* diferencia_altura_1, int* diferencia_altura_2, int resultado){
...
*/
void detectar_personaje(int* puntos,int* altura, int* diferencia_altura_1, int* diferencia_altura_2, int resultado){
	if ((*puntos) >= PTS_MIN_JHONNY_CORAJE && (*puntos) <= PTS_MAX_JHONNY_CORAJE){
		detectar_coraje_o_johnny(altura, diferencia_altura_1, diferencia_altura_2, resultado);
	}
	if ((*puntos) >= PTS_MIN_HUESO_POLLITO && (*puntos) <= PTS_MAX_HUESO_POLLITO){
		detectar_pollito_o_hueso(altura, diferencia_altura_1, diferencia_altura_2, resultado);
	}	
	if ((*puntos) >= PTS_MIN_BLUE_BELLOTA && (*puntos) <= PTS_MAX_BLUE_BELLOTA){
		detectar_bellota_o_blue(altura, diferencia_altura_1, diferencia_altura_2, resultado);
	}	
}
...
int main(){
	...
	detectar_personaje(&puntos,&altura,&diferencia_altura_1,&diferencia_altura_2,resultado);
